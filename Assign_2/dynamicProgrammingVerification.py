...
while any(amountInFleet > 0 for amountInFleet in Fleet.amount.values()):
            ...
            for i in range(len(stagesList)):
                ...
                for IATA, currentNode in currentStage.nodesList.items():
                    ...
                    for iterIATA, iterStageNumber in currentNode.vertices.items():
                        ...
                        OrigDestDemand = currentAirportDemand.loc[currentAirportDemand['To'] == destination.IATA].drop(
                            ['From', 'To'], axis=1)  # origin destination demand
    ...
    if all(value <= 0 for value in
           aircraftProfits.values()):  # if no aircraft type gives a profitable route, stop adding new aircraft
        break
    ...
    print("Saving aircraft route of aircraft type " + str(highestAircraftProfitType) + " with a profit of " + str(
        highestAircraftProfitValue) + '\n')
    ...
    while True:
        ...
        currentRouteNode = RouteNode(currentRouteNodeIATA, currentRouteStage.time, currentRouteStageNode.profit,
                                     currentRouteStageNode.cargo, currentRouteStageNode.stageNumber,
                                     currentRouteStage.binNumber)  # CARGO ADDED
    ...
    for i in range(len(routeFlights) - 1):
        ...
        if currentNode.cargo != 0:
            if currentNode.binNumber not in binFlightTime:
                ...
                binFlightTime[currentNode.binNumber] = [binFlightTime[currentNode.binNumber],
                                                        [currentNode.IATA, nextNode.IATA]]
            ...
            demand.loc[indices_OD] = demand.loc[indices_OD].replace(
                demand.loc[indices_OD].iloc[0, 2 + currentNode.binNumber], max(0, newBinFlightDemand))
            ...
            if currentNode.cargo > binFlightDemand:
                ...
                demand.loc[indices_OD] = demand.loc[indices_OD].replace(
                    demand.loc[indices_OD].iloc[0, 2 + currentNode.binNumber - 1], max(0, newPrevBinFlightDemand))
                ...
                if abs(newBinFlightDemand) > prevBinFlightDemand:
                    ...
                    demand.loc[indices_OD] = demand.loc[indices_OD].replace(
                        demand.loc[indices_OD].iloc[0, 2 + currentNode.binNumber - 2],
                        max(0, newPrevPrevBinFlightDemand))
                    binCargo_Node[currentNode.binNumber - 2] = prevPrevBinFlightDemand - max(0,
                                                                                             newPrevPrevBinFlightDemand)
                    ...
                    if abs(newPrevBinFlightDemand) > prevPrevBinFlightDemand:
                        ...
                        print(
                            f'Het gaat mis hier bij vlucht {currentNode.IATA} naar {nextNode.IATA}, bij bin {currentNode.binNumber}')
            ...
            if currentNode.binNumber - 1 in binFlightTime:  # Is there a flight in the previous bin?
                if [currentNode.IATA, nextNode.IATA] == binFlightTime[
                    currentNode.binNumber - 1]:  # Does that flight have the same origin-destination as the current flight?
                    if currentNode.binNumber - 1 in binCargo[
                        currentNode.binNumber]:  # Does the current flight carry demand from the previous bin?
                        oldPrevBinDemand = usedDemand.loc[
                            (usedDemand['From'] == binFlightTime[currentNode.binNumber - 1][0]) & (
                                        usedDemand['To'] == binFlightTime[currentNode.binNumber - 1][1])].iloc[
                            0, 2 + currentNode.binNumber - 1]  # What was the original demand at the previous bin?
                        totalPrevTransportedDemand = binCargo[currentNode.binNumber - 1][currentNode.binNumber - 1] + \
                                                     binCargo[currentNode.binNumber][
                                                         currentNode.binNumber - 1]  # How much demand is transported?
                        if totalPrevTransportedDemand > oldPrevBinDemand:  # Has there been transported more demand than available?
                            ...
                            flightRevenue = Financials.calculateRevenue(airportsList[origin], airportsList[destination],
                                                                        prevErrorCargo)
                            flightCost = Financials.calculateCost(airportsList[origin], airportsList[destination],
                                                                  highestAircraftProfitType)
            ...
            if currentNode.binNumber - 2 in binFlightTime:  # Is there a flight in the pre previous bin?
                if [currentNode.IATA, nextNode.IATA] == binFlightTime[
                    currentNode.binNumber - 2]:  # Does that flight have the same origin-destination as the current flight?
                    if currentNode.binNumber - 2 in binCargo[
                        currentNode.binNumber]:  # Does the current flight carry demand from the pre previous bin?
                        oldPrevPrevBinDemand = usedDemand.loc[
                            (usedDemand['From'] == binFlightTime[currentNode.binNumber - 2][0]) & (
                                        usedDemand['To'] == binFlightTime[currentNode.binNumber - 2][1])].iloc[
                            0, 2 + currentNode.binNumber - 2]  # What was the original demand at the previous bin?
                        totalPrevPrevTransportedDemand = binCargo[currentNode.binNumber - 2][
                                                             currentNode.binNumber - 2] + \
                                                         binCargo[currentNode.binNumber][
                                                             currentNode.binNumber - 2]  # How much demand is transported?
    ...
    if len(binErrorProfit) >= 1:
        print(
            f'To much demand is transported on the flight in the following bin, resulting in the following reduction in profit: {binErrorProfit}')
    else:
